odoo.define('pos_ticket_base_template.PaymentScreen', function (require) {
    'use strict';
    ...
    const PosTicketBaseTemplatePaymentScreen = PaymentScreen => class PosTicketBaseTemplatePaymentScreen extends PaymentScreen {
        /**
         * @override
         */
        async _finalizeValidation() {
            ...
            const module_pos_extend_receipt_app = await this.rpc({
                model: 'ir.module.module',
                method: 'search_read',
                args: [[['name', '=', 'pos_extend_receipt_app']], ['state']],
            });
            ...
            try {
                ...
                // 2. Invoice.
                if (this.shouldDownloadInvoice() && syncOrderResult[0]?.account_move) {
                    if (syncOrderResult.length) {
                        await this.env.pos.report_action_backend(syncOrderResult[0].account_move);
                    } else {
                ...
                // Flow module pos_extend_receipt_app
                if (module_pos_extend_receipt_app.length && module_pos_extend_receipt_app[0]['state'] == 'installed') {
                    if (this.currentOrder.is_to_invoice()) {
                        this.rpc({
                            model: 'pos.order',
                            method: 'search_read',
                            args: [[['pos_reference', '=', this.currentOrder['name']]], ['account_move']],
    ...
    Registries.Component.extend(PaymentScreen, PosTicketBaseTemplatePaymentScreen);
