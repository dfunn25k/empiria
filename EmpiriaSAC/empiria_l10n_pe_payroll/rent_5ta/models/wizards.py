...
class PayrollProjectionWizard(models.TransientModel):
    ...
    def calc_rent_5ta(self):
        ...
        for employee in employees:
            ...
            self.get_projection_type(projection_id, periods)
            ...
            self.get_amount_total_gratificacion(projection_id, periods)

            self.get_amount_rem_percibidas_meses_ant(projection_id,periods)
    ...
    def get_contract(self, employee, date_to, date_from):
        ...
        clause_previous = [('employee_id', '=', employee.id), ('state', '=', 'close'), '|',
                           '|'] + clause_1 + clause_2 + clause_3
        current_contract = self.env['hr.contract'].search([('employee_id', '=', employee.id), ('state', '=', 'open')])

        previous_contract = self.get_previous_contract(employee, current_contract.date_start)
        if previous_contract and self.env['hr.contract'].search(clause_previous) in previous_contract:
            return self.env['hr.contract'].search(clause_previous)

        return self.env['hr.contract'].search(clause_final)

    @api.model
    def get_contract_actual(self, employee, date_to, date_from):
        # a contract is valid if it ends between the given dates
        clause_1 = ['&', ('date_end', '<=', date_to), ('date_end', '>=', date_from)]
        # OR if it starts between the given dates
        clause_2 = ['&', ('date_start', '<=', date_to), ('date_start', '>=', date_from)]
    ...
    def get_previous_contract(self, employee, current_date):
        close_contracts = self.env['hr.contract'].search([('employee_id', '=', employee.id), ('state', '=', 'close')], order='date_start desc', )
        successive_contracts = []
        if close_contracts and close_contracts[0].date_end + timedelta(days=1) == current_date:
            successive_contracts.append(close_contracts[0])
        else:
            return

        for i in range(1, len(close_contracts)):
            if close_contracts[i - 1].date_start - timedelta(days=1) == close_contracts[i].date_end:
                if close_contracts[i - 1] not in successive_contracts:
                    successive_contracts.append(close_contracts[i - 1])
                if close_contracts[i] not in successive_contracts:
                    successive_contracts.append(close_contracts[i])
            else:
                break
        return successive_contracts
    @staticmethod
    ...
    def get_amount_rem_computable(self, employee, projection_id, periods):
        ...
        if self.projection_type == 'contract':
            hijoTot = self.env["hr.employee.relative"].search([('relation_id', 'in', (3,))])
            for period in periods:
                month = period[0:2]
                ...
                relatives = self.env["hr.employee.relative"].browse(employee.relative_ids)
                ind = False
                for relative in relatives:
                    if relative.id in hijoTot:
                        ind = True
                        break
                ...
                if ind:
                    ...
                    if rmv_id:
                        total += rmv_id.af_amount if contract_id else 0
    ...
    def get_amount_rem_meses_anteriores(self, projection_id, periods, employee):
        ...
        for period in periods:
            ...
            if self.projection_type == 'contract':
                for x in range(len(data_start_list)):
                    if data_start_list[x] not in payslip_ids.mapped('date_start'):
                        total += self.get_calc_per_month_project_line(
                            'rent_5ta.payroll_projection_exception_01', projection_id, data_start_list[x][:2])
            ...
            r3 = self.env.ref('rent_5ta.payroll_projection_exception_09', False)
            r4 = self.env.ref('rent_5ta.payroll_projection_exception_08', False)
            for payslip in payslip_ids:
                total += self.get_amount_validate_rule_exception(payslip, [r1, r2, r3, r4])
            data = self.update_dict_data(projection_line, month, total, data)
        self.update_project_line_records(data)
    ...
    def get_amount_grati(self, employee, projection_id, periods):
        ...
        if self.cancel_rent:
            ...
            hijo_tot = self.env["hr.employee.relative"].search([('relation_id', 'in', (3,))])
            for period in periods:
                date_from, date_to = self.env['hr.payslip'].get_month_day_range(period)
                contract_id = self.get_contract_actual(employee, date_from, date_to)
                previous_contract = self.get_previous_contract(employee, employee.service_start_date)

                month = period[0:2]
                last_month = "{:02d}".format(int(month))
                int_last_month = int(last_month)
                data_start_list = []
                ...
                if contract_id and contract_id[0].labor_regime_id:
                    ...
                    # browse if employee have a children in relatives_ids field
                    relatives = self.env["hr.employee.relative"].browse(employee.relative_ids)
                    children = False
                    for relative in relatives:
                        if relative.id in hijo_tot:
                            children = True
                            break
                        ...
                        if contract_id[0].labor_regime_id.code == '17':
                            if qty == 0:
                                ...
                                if previous_contract:
                                    date_start_first = previous_contract[-1].date_start
                                else:
                                    date_start_first = employee.service_start_date

                                if date_start_first and date_start_first > projection_id.date_from:
                                    diff_days = (date_start_first - projection_id.date_from).days
                                ...
                                if 1 <= int(month) <= 6:
                                    ...
                                    if date_start_first and date_start_first > first_day:
                                        diff_days = (date_start_first - first_day).days
                                result2 = result1 - (result1 / 2 / 6 / 30 * diff_days)
                                total = value + result2
                            ...
                            if qty == 0:
                                ...
                                if 1 <= int(month) <= 6:
                                    ...
                                    if date_start_first and date_start_first > first_day:
                                        diff_days = (date_start_first - first_day).days
                                result2 = result1 - (result1 / 6 / 30 * diff_days)
                                total = value + result2
        ...
        for period in periods:
            ...
            if total == 0:
                ...
                total += self.get_calc_per_month_project_line('rent_5ta.payroll_projection_exception_06', projection_id,
                                                              month)
                total += self.get_calc_per_month_project_line('rent_5ta.payroll_projection_exception_total_grati', projection_id,
                                                              month)
                total += self.get_calc_per_month_project_line('rent_5ta.payroll_projection_exception_10', projection_id,
                                                              month)
                total += self.get_calc_per_month_project_line('rent_5ta.payroll_projection_exception_10_meses_ant', projection_id,
                                                              month)
                total += self.get_calc_per_month_project_line('rent_5ta.payroll_projection_exception_11', projection_id,
                                                              month)
                total += self.get_calc_per_month_project_line('rent_5ta.payroll_projection_exception_year',
                                                              projection_id, month)
    ...
    def get_amount_dscto_quinta(self, period, employee):
        total = 0
        payslip_ids = self.env['hr.payslip'].search([
            ('employee_id', '=', employee.id),
            ('date_start', '=', period),
    ...
    def get_amount_retenciones_meses_ant(self, projection_id, periods, employee):
        ...
        for period in periods:
            ...
            if self.projection_type in ['contract','current_month']:
                if month in ['04', '08', '12']:
                    last_month = "{:02d}".format(int(month) - 1)
                    int_last_month = int(last_month)
                    total = 0
                    while int_last_month > 0:
                    ...
                    last_month = "{:02d}".format(int('05') - 1)
                    int_last_month = int(last_month)
                    total = 0
                    while int_last_month > 0:
                        last_month = "{:02d}".format(int_last_month)
                    ...
                    last_month = "{:02d}".format(int('09') - 1)
                    int_last_month = int(last_month)
                    total = 0
                    while int_last_month > 0:
                        last_month = "{:02d}".format(int_last_month)
            ...
            if data_start_list:
                if self.projection_type == 'contract':
                    for x in range(len(data_start_list)):
                        if (self.get_amount_dscto_quinta(data_start_list[x], employee) == 0):
                            total += self.get_calc_per_month_project_line(
                                'rent_5ta.payroll_projection_exception_total_r2_3', projection_id, data_start_list[x][:2])
                        else:
                            total += self.get_amount_dscto_quinta(data_start_list[x], employee)
                else:
                    payslip_ids = self.env['hr.payslip'].search([
                        ('employee_id', '=', employee.id),
                        ('date_start', 'in', data_start_list),
    ...
    def get_amount_total_renta_neta(self, projection_id, periods):
        ...
        for period in periods:
            month = period[0:2]
            total = 0.0
            total += self.get_calc_per_month_project_line('rent_5ta.payroll_projection_exception_r2_1',
                                                          projection_id, month)
            total += self.get_calc_per_month_project_line('rent_5ta.payroll_projection_exception_09',
                                                          projection_id, month)
            total += self.get_calc_per_month_project_line('rent_5ta.payroll_projection_exception_08',
                                                          projection_id, month)
    ...
    def get_amount_retencion_afectada_extraordinarias(self, projection_id, periods):
        ...
        for period in periods:
            ...
            projection_line = projection_id.line_ids.filtered(lambda x: x.exception_id == name_exception)
            data = self.update_dict_data(projection_line, month, total, data, integer=False)
        self.update_project_line_records(data)
    ...
    def get_amount_rem_percibidas_meses_ant(self, projection_id, periods):
        data = []
        for period in periods:
            month = period[0:2]
            last_month = "{:02d}".format(int(month) - 1)
            int_last_month = int(last_month)
            name_exception = self.env.ref('rent_5ta.payroll_projection_exception_10_meses_ant', False)
            projection_line = projection_id.line_ids.filtered(lambda x: x.exception_id == name_exception)
            total = 0
            data_start_list = []
            while int_last_month > 0:
    ...
    def get_amount_total_gratificacion(self, projection_id, periods):
        data = []
        for period in periods:
            month = period[0:2]
            total = self.get_calc_per_month_project_line(
                'rent_5ta.payroll_projection_exception_07', projection_id, month)
            total += self.get_calc_per_month_project_line('rent_5ta.payroll_projection_exception_ajus_grati', projection_id,
                                                              month)
            name_exception = self.env.ref('rent_5ta.payroll_projection_exception_total_grati', False)
            projection_line = projection_id.line_ids.filtered(lambda x: x.exception_id == name_exception)

            data = self.update_dict_data(projection_line, month, total, data)
        self.update_project_line_records(data)

    def get_projection_type(self, projection_id, periods):
        data = []
        month_mapping = {
            '01': 'january_amount',
            '02': 'february_amount',
        ...
        for period in periods:
            ...
            if self.projection_type == 'contract':
                projection = 'Contrato'
            elif self.projection_type == 'last_month':
                projection = 'Mes anterior'
            elif self.projection_type == 'current_month':
                projection = 'Mes actual'
            else:
                projection = ''

            name_exception = self.env.ref('rent_5ta.payroll_projection_exception_type', False)
            projection_line = projection_id.line_ids.filtered(lambda x: x.exception_id == name_exception)

            month_key = month_mapping.get(month)
            if month_key:
                sub_dict = {month_key: projection}
                value = {projection_line.id: sub_dict}
                data.append(value)
