...
class SirePurchaseNationalCustomTwo(models.TransientModel):
    _name = "sire.purchase.national.custom.two"
    _inherit = "sire.purchase.base"
    _description = "SIRE Reporte RCE - Compras Nacionales Personalizado 2"
    ...
    def _select_sire_purchase(self):
        ...
        - `detraction_voucher_payment_date`     : Fecha de pago del voucher de detracción, formateada como 'DD/MM/YYYY', si existe una detracción.
        - `detraction_voucher_number`           : Número de voucher de la detracción, si existe una detracción.
        ...
        return (
            ...
            + """,            
                -- Fecha de pago del voucher de detracción, formateada como 'DD/MM/YYYY'
                CASE
                    WHEN account_move.detraction_id IS NOT NULL THEN
                        COALESCE(TO_CHAR(account_move.voucher_payment_date, 'DD/MM/YYYY'), '') 
                    ELSE ''
                END AS detraction_voucher_payment_date,

                -- Número del voucher de detracción
                CASE
                    WHEN account_move.detraction_id IS NOT NULL THEN
                        COALESCE(account_move.voucher_number, '')
                    ELSE ''
                END AS detraction_voucher_number
            """
        )
    ...
    def _return_view_sire_purchase(self):
        wizard_form_id = self.env.ref(
            "l10n_pe_sire_purchase_national_custom_two.view_sire_rce_report_custom_two_national_purchase_form"
        ).id
        return {
            "type": "ir.actions.act_window",
            "view_type": "form",
            "view_mode": "form",
            "res_model": "sire.purchase.national.custom.two",
