...
from ..reports.sire_purchase_accept_proposed import (
    SirePurchaseAcceptProposedXlsx,
    SirePurchaseAcceptProposedTxt,
)
from ..reports.sire_purchase_replace_proposed import (
    SirePurchaseReplaceProposedXlsx,
    SirePurchaseReplaceProposedTxt,
)
from ..reports.sire_purchase_subsequent_adjustments import (
    SirePurchaseSubsequentAdjustmentsXlsx,
    SirePurchaseSubsequentAdjustmentsTxt,
)
from ..reports.sire_purchase_general_format import (
    SirePurchaseGeneralFormatXlsx,
    SirePurchaseGeneralFormatTxt,
)


class SirePurchaseNationalCustom(models.TransientModel):
    _name = "sire.purchase.national.custom"
    _inherit = "sire.purchase.base"
    _description = "SIRE Reporte RCE - Compras Nacionales Personalizado"
    ...
    opportunity_code = fields.Selection(
        selection=[
            ...
            (
                "04",
                "[04] Reporte de ajustes posteriores de periodos anteriores al nuevo sistema de registros - Formato general",
            ),
        ],
    ...
    def _select_sire_purchase(self):
        """
        Extiende la selección de columnas SQL para incluir detalles adicionales relacionados con la detracción
        y la distribución contable.

        Se agregan las siguientes columnas al conjunto de resultados:
        - `aggregated_account_codes`            : Códigos de cuenta asociados a las líneas de movimiento que contienen productos.
        - `aggregated_analytic_account_names`   : Nombres de cuentas analíticas asociadas a las distribuciones de analítica.

        Returns:
            str: La extensión de las columnas SQL con los detalles relacionados con la detracción y la distribución contable.
        """
        return (
            super()._select_sire_purchase()
            + """,
                -- Nombres de cuentas analíticas basadas en la distribución de analítica
                COALESCE(
                    (
                        SELECT 
                            STRING_AGG(analytic_account.name, ', ') AS aggregated_analytic_account_names
                        FROM 
                            account_analytic_account AS analytic_account
                        WHERE 
                            analytic_account.id IN (
                                SELECT 
                                    DISTINCT (jsonb_object_keys(move_line.analytic_distribution))::INTEGER AS distribution_account_ids
                                FROM 
                                    account_move_line AS move_line
                                WHERE 
                                    move_line.move_id = account_move.id
                                    AND move_line.product_id IS NOT NULL
                            )
                    ), ''
                ) AS aggregated_analytic_account_names, 
                
                -- Códigos de cuentas asociadas a las líneas de movimiento con productos
                COALESCE(
                    (
                        SELECT
                            STRING_AGG(DISTINCT account_account.code, ', ') AS aggregated_account_codes
                        FROM
                            account_move_line AS move_line
                            INNER JOIN account_account AS account_account ON move_line.account_id = account_account.id
                        WHERE
                            move_line.move_id = account_move.id
                            AND move_line.product_id IS NOT NULL
                    ),
                    ''
                ) AS aggregated_account_codes                                   
            """
        )

    def _where_sire_purchase(self):
        return (
            super()._where_sire_purchase()
            + """
            AND (
    ...
    def _generate_xlsx_file(self, processed_results):
        if self.opportunity_code == "01":
            ...
            raise ValidationError(
                "Por favor, se necesita un código de oportunidad para generar el reporte, gracias."
            )
        return {
    ...
    def _generate_zip_file(self, txt_filename, txt_content):
        ...
        return {"zip_binary": zip_binary, "zip_filename": zip_filename}
    ...
    def _generate_files(self):
        ...
        zip_values = self._generate_zip_file(
            txt_values.get("txt_filename"), txt_values.get("txt_content", "\n")
        )
        return {
    ...
    def _save_generate_files(self, file_values):
        ...
        error_dialog = (
            "No hay contenido que presentar para este periodo."
            if not has_content_values
            else ""
        )
        self.write(
            {
                "xlsx_binary": base64.b64encode(
                    file_values["xlsx_values"].get("xlsx_content")
                ),
    ...
    def _return_view_sire_purchase(self):
        wizard_form_id = self.env.ref(
            "l10n_pe_sire_purchase_national_custom.view_sire_rce_report_custom_national_purchase_form"
        ).id
        return {
            "type": "ir.actions.act_window",
            "view_type": "form",
            "view_mode": "form",
            "res_model": "sire.purchase.national.custom",
