...
class SirePurchaseSubsequentAdjustmentsXlsx:
    def __init__(self, obj, processed_results):
        self.obj = obj
        self.processed_results = processed_results

    def __str__(self):
        return "Reporte RCE nacional - Cuando realiza ajustes posteriores XLSX - {}, {}{}".format(
            self.obj.company_id.name, self.obj.year, self.obj.month
        )
    ...
    def _write_headers_report(workbook, worksheet):
        ...
        worksheet.write(0, 37, "CAR Orig", style_header)
        worksheet.write(0, 38, "Detracción", style_header)
        worksheet.write(0, 39, "", style_header)
        worksheet.write(0, 40, "", style_header)
        worksheet.write(0, 41, "", style_header)
        worksheet.write(0, 42, "Analíticos", style_header)
        worksheet.write(0, 43, "Cuentas", style_header)
    ...
    def _write_rows_report(workbook, worksheet, processed_results):
        ...
        for dict_result in processed_results:
            car_orig = "{}{}{}{}".format(
                dict_result["partner_vat"],
                dict_result["document_type_code"],
                dict_result["ref_serie"],
                str(dict_result["ref_correlative"]).zfill(10),
            ).strip()
            ...
            worksheet.write(
                row, 12, dict_result["partner_identification_code"], style_content
            )
            worksheet.write(row, 13, dict_result["partner_vat"])
            worksheet.write(row, 14, dict_result["partner_name"])
            worksheet.write(
                row, 15, dict_result["p_base_gdg"], style_number_two_decimal
            )
            worksheet.write(row, 16, dict_result["p_tax_gdg"], style_number_two_decimal)
            worksheet.write(
                row, 17, dict_result["p_base_gdm"], style_number_two_decimal
            )
            worksheet.write(row, 18, dict_result["p_tax_gdm"], style_number_two_decimal)
            worksheet.write(
                row, 19, dict_result["p_base_gdng"], style_number_two_decimal
            )
            worksheet.write(
                row, 20, dict_result["p_tax_gdng"], style_number_two_decimal
            )
            ...
            worksheet.write(
                row, 23, dict_result["p_tax_icbp"], style_number_two_decimal
            )
            worksheet.write(
                row, 24, dict_result["p_tax_other"], style_number_two_decimal
            )
            worksheet.write(
                row, 25, dict_result["amount_total"], style_number_two_decimal
            )
            worksheet.write(row, 26, dict_result["currency_name"], style_content)
            worksheet.write(
                row, 27, dict_result["exchange_rate"], style_number_three_decimal
            )
            worksheet.write(row, 28, dict_result["origin_invoice_date"], style_date)
            worksheet.write(
                row, 29, dict_result["origin_document_type_code"], style_content
            )
            ...
            worksheet.write(
                row, 32, dict_result["origin_number_correlative"], style_content
            )
            worksheet.write(
                row, 33, dict_result["classification_services_code"], style_content
            )
            worksheet.write(row, 34, "", style_content)
            worksheet.write(row, 35, "", style_content)
            worksheet.write(row, 36, "", style_content)
            worksheet.write(row, 37, car_orig)
            ...
            worksheet.write(row, 41, "", style_content)
            worksheet.write(
                row, 42, dict_result["aggregated_analytic_account_names"], style_content
            )

            # Extraer y dividir los códigos de cuenta agregados de la clave "aggregated_account_codes"
            aggregated_account_codes = dict_result["aggregated_account_codes"].split(
                ", "
            )

            # Establecer la columna inicial donde se empezará a escribir
            starting_column = 43

            # Iterar sobre los códigos de cuenta y escribirlos en columnas consecutivas
            for col_offset, account_code in enumerate(aggregated_account_codes):
                # Escribir cada código de cuenta en la fila actual y la columna correspondiente
                worksheet.write(
                    row, starting_column + col_offset, account_code, style_content
                )
            row += 1
...
class SirePurchaseSubsequentAdjustmentsTxt:
    def __init__(self, obj, processed_results):
        self.obj = obj
        self.processed_results = processed_results

    def __str__(self):
        return "Reporte RCE nacional - Cuando realiza ajustes posteriores TXT - {}, {}{}".format(
            self.obj.company_id.name, self.obj.year, self.obj.month
        )
    ...
    def _get_template_report():
        return (
            ...
            "{code_aduana}|{origin_number_correlative}|{classification_services_code}||||{car_orig}|"
            "{detraction}||||{aggregated_analytic_account_names}|{aggregated_account_codes}|\r\n"
        )
    ...
    def _write_template_report(template, processed_results):
        ...
        for dict_result in processed_results:
            car_orig = "{}{}{}{}".format(
                ...
                dict_result["document_type_code"],
                dict_result["ref_serie"],
                str(dict_result["ref_correlative"]).zfill(8),
            ).strip()
            content += template.format(
                ...
                exchange_rate=(
                    "{0:.3f}".format(dict_result["exchange_rate"])
                    if dict_result["exchange_rate"]
                    else ""
                ),
                ...
                classification_services_code=dict_result[
                    "classification_services_code"
                ],
                car_orig=car_orig,
                detraction=dict_result["detraction"],
                aggregated_analytic_account_names="",
                aggregated_account_codes="",
            )
        return content
