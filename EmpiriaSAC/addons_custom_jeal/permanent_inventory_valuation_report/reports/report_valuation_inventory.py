...
class LedgerReportExcel(object):
    ...
    def get_content(self):
        ...
        style_content = workbook.add_format(
            {"valign": "vcenter", "size": 10, "border": 7}
        )
        style_number = workbook.add_format(
            {"size": 10, "num_format": "#,##0.00", "border": 7}
        )
    ...
    def get_content(self):
        ...
        for value in self.data:
            raw += template.format(
                ...
                quantity_hand_accumulated="{0:.2f}".format(
                    value["quantity_hand_accumulated"]
                ),
                cost_unit_accumulated="{0:.2f}".format(value["cost_unit_accumulated"]),
                cost_total_accumulated="{0:.2f}".format(
                    value["cost_total_accumulated"]
                ),
                state=value["state"],
            )
        return raw
