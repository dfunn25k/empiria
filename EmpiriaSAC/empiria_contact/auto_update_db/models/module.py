...
class Module(models.Model):
    ...
    def avoid_studio_customization_update(self):
        self.env.cr.execute("SELECT id FROM ir_module_module WHERE name = 'studio_customization'")
        studio_exists = [r[0] for r in self.env.cr.fetchall()]
        if studio_exists:
            self.env.cr.execute(f"update ir_module_module set state='installed' where id={studio_exists[0]}")
            _logger.info(f"Desactivar actualización modulo studio_customization. ID: {studio_exists[0]}")
    ...
    def upgrade_changed_checksum(self, overwrite_existing_translations=False):
        ...
        self.update_list()
        self.avoid_studio_customization_update()
