...
class HrEmployee(models.Model):
    ...
    unamapped_attendance_device_ids = fields.Many2many(
        'attendance.device', 'device_employee_rel', 'employee_id', 'device_id',
    ...
    created_from_attendance_device = fields.Boolean(
        string='Created from Device', readonly=True, groups="hr.group_hr_user",
        help='This field indicates that the employee was created from the data of an attendance device')
    finger_templates_ids = fields.One2many(
        'finger.template', 'employee_id', string='Finger Template', readonly=True,
        groups='hr.group_hr_user'
    )
    total_finger_template_records = fields.Integer(
        string='Finger Templates', compute='_compute_total_finger_template_records',
        groups='hr.group_hr_user'
    )
    device_user_ids = fields.One2many(
        'attendance.device.user', 'employee_id', string='Mapped Device Users',
        groups='hr.group_hr_user'
    )
