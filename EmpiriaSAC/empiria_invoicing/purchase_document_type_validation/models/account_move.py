...
def _validate_word_structure(word, validation_type, field_name, rec):
    special_characters = '-°%&=~\\+?*^$()[]{}|@%#"/¡¿!:.,;'
    if word:
        ...
        special = ''
        for letter in word:
            if letter in special_characters and rec.currency_id.name == 'PEN':
                special += letter
        if special != '':
        ...
        for rec in self:
            if rec.move_type not in ['out_invoice', 'out_refund'] and rec.name:
                ...
                if reference and '-' in reference and len(reference.split('-')) == 2 and rec.l10n_latam_document_type_id:
                    ...
                    msg += _validate_word_structure(serie, rec.l10n_latam_document_type_id.prefix_validation, 'Serie',rec)
                    msg += _validate_word_structure(correlative, rec.l10n_latam_document_type_id.correlative_validation, 'Correlativo',rec)
                else:
                    if rec.move_type != 'entry' and reference and '-' in reference and len(reference.split('-')) != 2 and rec.currency_id.name == 'PEN':
