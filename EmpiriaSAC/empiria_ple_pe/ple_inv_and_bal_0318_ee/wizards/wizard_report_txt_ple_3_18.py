            ...
            if account_id.account_type != 'asset_cash':
                for efemdcat in account_id.efemd_category:
                    ...
                    if len(data_txt_dict) > 0 and temp_index != -1:
                        ...
                        # Verificación segura de la jerarquía de padres
                        parent_1 = efemdcat.parent_ids[0] if efemdcat.parent_ids else None
                        parent_2 = parent_1.parent_ids[0] if parent_1 and parent_1.parent_ids else None
                        parent_3 = parent_2.parent_ids[0] if parent_2 and parent_2.parent_ids else None
                        
                        data_txt_dict.append({
                            'code': efemdcat.code,
                            'hierarchy': 'HIJO',
                            'balance': move_line.balance * (-1),
                            'parent_code': parent_1.code if parent_1 else '',
                            'grandparent_code': parent_2.code if parent_2 else '',
                            'greatgrandparent_code': parent_3.code if parent_3 else ''
                        })
