...
class PlePermanentInventoryPhysicalUnits(models.Model):
    ...
    def action_generate_excel(self):
        ...
        query = """
                ...
                CASE WHEN sp.picking_number != '' AND LENGTH(sp.picking_number) > LENGTH(REPLACE(sp.picking_number, '-', '')) THEN SPLIT_PART(sp.picking_number, '-', LENGTH(sp.picking_number) - LENGTH(REPLACE(sp.picking_number, '-', '')))
                ELSE 
    ...
    def opening_balance_units(self, product, quantity_hand, year, month, day, correct_name=False):
        ...
        cuo = f"SALDOINICIAL{year}{str(month).zfill(2)}{prod.id or 0}{svl.id or 0}"
        valuation_date = f"{str(day).zfill(2)}/{str(month).zfill(2)}/{year}"
    
        datos = {
            'period': period,
            'cuo': cuo,
            'correlative': 'M000000001',
            'annexed_establishment_code': (self.company_id.partner_id.annexed_establishment or '') if self.company_id and self.company_id.partner_id else '',
            'stock_catalog': prod.stock_catalog or '' if prod else '',
            'stock_type': prod.stock_type or '' if prod else '',
            'stock_own_code': prod.default_code or '' if prod else '',
            'catalog_code': code_catag,
            'stock_code': (prod.unspsc_code_id.code or '') if (prod and prod.unspsc_code_id) else '',
            ...
            'unit_measure_code': quantity_hand.get('udm_product') if isinstance(quantity_hand, dict) else quantity_hand,
            'qty_physical_units_asset_entered': quantity_hand.get('quantity_product_hand', 0.00) if isinstance(quantity_hand, dict) else 0.00,
