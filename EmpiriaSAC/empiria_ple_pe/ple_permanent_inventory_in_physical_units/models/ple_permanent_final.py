...
class PlePermanentFinal(models.Model):
    ...
    def opening_balances(self, product, quantity_hand, year, month, day, correct_name=False):
        ...
        prod = product if getattr(product, '_name', '') == 'product.product' else Product.browse(int(product) if product else False)
        if not prod or not prod.exists():
            prod = Product.browse(False)
    
        svl = self.env['stock.valuation.layer'].search([('product_id', '=', prod.id or 0)], order='id asc', limit=1)
    
        q_hand = (quantity_hand.get('quantity_product_hand', 0.0) if isinstance(quantity_hand, dict) else 0.0)
        divisor = 1 if round(q_hand, 2) == 0 else q_hand
    
        code_catag = '1' if (prod and prod.unspsc_code_id) else ''
    
        if not correct_name:
            name_source = (prod.display_name or prod.name or '') if prod else ''
            correct_name = name_source.split(']')[-1].strip()[:80]
        else:
            correct_name = str(correct_name).strip()[:80]
    
        period = f"{year}{str(month).zfill(2)}00"
        cou = f"SALDOINICIAL{year}{str(month).zfill(2)}{prod.id or 0}{svl.id or 0}"
        date_start = f"{str(day).zfill(2)}/{str(month).zfill(2)}/{year}"
    
        default_code = ''
        if prod:
            default_code = prod.default_code or ''
            default_code = re.sub(r'[^A-Za-z0-9]+', '', default_code)
    
        total_value = (quantity_hand.get('total_value', 0.0) if isinstance(quantity_hand, dict) else 0.0)
        standard_price = (quantity_hand.get('standard_price', 0.0) if isinstance(quantity_hand, dict) else 0.0)
    
        datos = {
            'period': period,
            'cou': cou,
            'correlativo': 'M000000001',
            'establishment': (self.company_id.partner_id.annexed_establishment or '') if self.company_id and self.company_id.partner_id else '',
            'catalog': (prod.stock_catalog or '') if prod else '',
            'stock_type': (prod.stock_type or '') if prod else '',
            'default_code': default_code,
            'code_catag': code_catag,
            'unspsc_code': (prod.unspsc_code_id.code or '') if (prod and prod.unspsc_code_id) else '',
            ...
            'uom': quantity_hand.get('udm_product') if isinstance(quantity_hand, dict) else quantity_hand,
            'code_exist': quantity_hand.get('code_exist', '') if isinstance(quantity_hand, dict) else '',
            'quantity_product_hand': q_hand,
            'standard_price': standard_price,
            'total_value': total_value,
            'quantity': 0.00,
            'unit_cost': 0.00,
            'value_cost': 0.00,
            'quantity_hand_accumulated': round(q_hand, 2),
            'cost_unit_accumulated': round(total_value / divisor, 2),
            'cost_total_accumulated': round(total_value, 2),
            'state': '1',
            'header': True,
        }
        return datos
    ...
    def opening_balances(self, product, quantity_hand, year, month, day, correct_name=False):
        ...
        if not correct_name and product_product.display_name:
