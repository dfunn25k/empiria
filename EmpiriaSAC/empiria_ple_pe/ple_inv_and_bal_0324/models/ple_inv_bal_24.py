...
from ..reports.report_inv_bal_24 import ReportInvBal24Excel, ReportInvBal24Txt
import base64

class PleInvBal24(models.Model):
    _name = 'ple.report.inv.bal.24'
    _description = 'Statements Comprehensive Income'
    _inherit = 'ple.report.base'

    line_ids = fields.One2many(
        comodel_name='ple.report.inv.bal.line.24',
        inverse_name='ple_report_inv_val_24_id',
        string='Líneas'
    )
    ...
    txt_binary = fields.Binary(
        string='Reporte .TXT 3.24'
    )
    pdf_filename = fields.Char(
        string='Reporte .PDF 3.24'
    )
    ...
    def action_generate_report(self):
        ...
        account_query = """
                ...
                --  TYPE JOIN   |  TABLE                        | MATCH
                    INNER JOIN   account_account                ON account_account.eri_ple_id = eeff_ple.id
                -- FILTER QUERIES 
                    WHERE eeff_ple.eeff_type = '3.24';                    
        """
        self.env.cr.execute(account_query)
        val_account = self.env.cr.dictfetchall()

        if not val_account:
            self.write({'error_dialog': 'No hay cuentas configuradas con tipo 3.24 ERI'})
            return True
        ...
        SELECT
            ...
            -- FILTER QUERIES 
                ...
                "account_move_line"."account_id" in( SELECT "account_account".id FROM "account_account"
                 WHERE ("account_account"."account_type" in (SELECT "account_account".account_type FROM "account_account" 
        ...
        try:
            ...
            lines_data = {}
            for dict in values:
                self.check_key_in_dicts(dict['eri_ple_id'], lines_data, dict)
            ...
            for data in lines_report:
                if len(data) == 10:
                    del data['parent']
            self.env['ple.report.inv.bal.line.24'].create(lines_report)

        except Exception as error:
            raise ValidationError(f'Error al ejecutar la query, comunicar al administrador: \n {error}')

    def check_parent_lines(self, eri_ple_id, credit, list_data):
        if isinstance(eri_ple_id, int):
            eeff_ple_data = self.env['eeff.ple'].search([('id', '=', eri_ple_id)])
        else:
            eeff_ple_data = eri_ple_id
        ...
        if not parent_ids:
            ...
            for parent_id in parent_ids:
                parent_values = {
                    ...
                    'eri_ple_id': parent_id.id or False,
                    'parent': parent_id.id or False,
                    'credit': credit,
                    'state': '1',
                    'ple_report_inv_val_24_id': self.id
                }
    ...
    def action_generate_excel(self):
        if self.action_generate_report():
            return
        list_data = []
        line_ids = self.env['ple.report.inv.bal.line.24'].search([('ple_report_inv_val_24_id', '=', self.id)],
                                                                 order='sequence asc')
        for obj_line in line_ids:
            ...
            values = {
                ...
                'eri_ple_id': obj_line.eri_ple_id,
                'account_ids': obj_line.account_ids,
                'real_credit': round(real_credit, 2),
            }
            list_data.append(values)

        report_txt = ReportInvBal24Txt(self, list_data)
        report_xls = ReportInvBal24Excel(self, list_data)
        ...
        for rec in self:
            report_name = "ple_inv_and_bal_0324.action_print_status_finance"
            pdf = self.env.ref(report_name, False)._render_qweb_pdf('ple_inv_and_bal_0324.print_status_finance',self.id)[0]
            rec.pdf_binary = base64.encodebytes(pdf)
            year, month, day = self.date_end.strftime('%Y/%m/%d').split('/')
            rec.pdf_filename = f'Libro_Estado de Resultados Integrales_{year}{month}.pdf'
