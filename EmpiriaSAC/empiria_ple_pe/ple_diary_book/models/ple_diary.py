...
class PleDiary(models.Model):
    ...
    def action_generate_excel(self):
        query_aml = """
                ...
                CASE 
                  WHEN account_journal.type = 'sale' AND account_move_line__move_id.move_type IN ('out_invoice', 'out_refund') THEN
                  ...
                  WHEN account_journal.type = 'purchase' AND account_move_line__move_id.move_type IN ('in_invoice', 'in_refund') THEN
                  ...
                  WHEN account_journal.type IN ('cash', 'bank', 'general') AND account_move_line__move_id.move_type = 'entry' THEN
        ...
        query_account = """
                ...
                COALESCE(
                    TRIM(SUBSTRING(
                            CASE
                                WHEN RIGHT(TRIM(account_account.name::text), 1) = '}}' AND LEFT(TRIM(account_account.name::text), 1) = '{{' THEN
                                    COALESCE(
                                        (account_account.name::jsonb->>'en_US'),
                                        (account_account.name::jsonb->>'es_PE'),
                                        account_account.name::text
                                    )
                                ELSE account_account.name::text
                            END, 1, 99)
                    ), '') as account_name,
                ...
                COALESCE(
                    TRIM(SUBSTRING(
                            CASE
                                WHEN RIGHT(TRIM(account_group.name::text), 1) = '}}' AND LEFT(TRIM(account_group.name::text), 1) = '{{' THEN
                                    COALESCE(
                                        (account_group.name::jsonb->>'en_US'),
                                        (account_group.name::jsonb->>'es_PE'),
                                        account_group.name::text
                                    )
                                ELSE account_group.name::text
                            END, 1, 59)
                    ), '') as name_group,
    ...
    def action_generate_report(self, data_aml, data_account):
        ...
        for obj_move_line in data_aml:
            ...
            nueva_lista = list(map(lambda clave: [elemento[1] for elemento in account_analytic_data if str(elemento[0]) == clave][0] if any(str(elemento[0]) == clave for elemento in account_analytic_data) else '', list(analytic_distribution2)))
            ...
            values_move = {
                ...
                'invoice_document_type_code': invoice_document_type_code,
