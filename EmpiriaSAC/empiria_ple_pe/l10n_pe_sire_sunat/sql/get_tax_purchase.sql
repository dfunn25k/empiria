...
BEGIN
    FOR aml_line IN (SELECT * FROM account_move_line WHERE move_id = account_move_id AND parent_state != 'cancel') LOOP
        FOR tax_row_line IN (SELECT * FROM account_account_tag WHERE 
            ...
            tax_name := tax_row_line.name::text;
            IF LEFT(TRIM(tax_name), 1) = '{' AND RIGHT(TRIM(tax_name), 1) = '}'
            THEN
                tax_name := COALESCE(
                    (tax_row_line.name::jsonb)->>'en_US',
                    (tax_row_line.name::jsonb)->>'es_PE',
                    tax_name
                );
            END IF;
            tax_name := REPLACE(REPLACE(tax_name, '-', ''), '+', '');
    ...
    SELECT COALESCE(SUM(
            CASE 
                WHEN move_type IN ('in_refund', 'in_invoice') THEN 
                    CASE 
                        WHEN aml_line.balance < 0 THEN ABS(aml_line.balance) 
                        ELSE aml_line.balance * -1 
                    END 
                ELSE 
                    aml_line.balance 
            END
    ...
    JOIN account_account account_account ON account_move_line.account_id = account_account.id
    WHERE account_move_line.move_id = account_move_id
    AND account_move_line.parent_state != 'cancel'
    AND account_account.group_id IN (SELECT id FROM account_group WHERE code_prefix_start = '42');
