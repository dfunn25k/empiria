...
class SireSaleReplaceProposedXlsx:
    def __init__(self, obj, processed_results):
        self.obj = obj
        self.processed_results = processed_results

    def __str__(self):
        return "Reporte RVIE - Cuando reemplaza la propuesta XLSX - {}, {}{}".format(
    ...
    def _write_rows_report(workbook, worksheet, processed_results):
        ...
        for dict_result in processed_results:
            ...
            worksheet.write(row, 22, dict_result['s_tax_ivap'], style_number_two_decimal)
            worksheet.write(row, 23, dict_result['s_base_ivap'], style_number_two_decimal)
            worksheet.write(row, 24, dict_result['s_tax_icbp'], style_number_two_decimal)
...
class SireSaleReplaceProposedTxt:
    def __init__(self, obj, processed_results):
        self.obj = obj
        self.processed_results = processed_results

    def __str__(self):
        return "Reporte RVIE - Cuando reemplaza la propuesta TXT - {}, {}{}".format(
            self.obj.company_id.name,
            self.obj.year,
            self.obj.month
        )
    ...
    def _get_template_report():
        return '{company_vat}|{company_name}|' \
            ...
            '{s_tax_ivap}|{s_base_ivap}|{s_tax_icbp}|{s_tax_other}|' \
