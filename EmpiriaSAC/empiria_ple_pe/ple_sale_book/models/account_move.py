...
class AccountMove(models.Model):
    ...
    def _get_view(self, view_id=None, view_type="form", **options):
        ...
        if view_type in ("form"):
            ...
            arch, view = self.env["res.partner"]._tags_invisible_per_country(
                arch, view, tags, [self.env.ref("base.pe")]
            )
        return arch, view
    ...
    def update_cancel_ple_state(self, values):
        ...
        document_type_id = (
            ...
            and self.l10n_latam_document_type_id.code in ["01", "03"]
        )
        if (
            self.move_type in ["out_invoice", "out_refund"]
            and document_type_id
            and values.get("state")
            ...
            if "ple_date" in values:
                ...
                for recs in self:
                    if recs.invoice_date and recs.date:
                        if recs.invoice_date.month < int(
                            ple_month[1]
                        ) or recs.invoice_date.year < int(ple_month[0]):
                            values["ple_state"] = "8"
                        else:
                            values["ple_state"] = "1"
