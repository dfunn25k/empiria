...
class PleReportSale(models.Model):
    ...
    move_ids = fields.Many2many(
        comodel_name="account.move", string="Facturas relacionadas"
    )
    ...
    def action_generate_excel(self):
        query = """
                    SELECT
                        ...
                        (
                            SELECT
                                CASE
                                    ...
                                    SELECT
                                        COALESCE(ple_correlative, 'M000000001')
                                    FROM
                                        account_move_line
                                    where
                                        move_id = am.id
                        ...
                        CASE
                            WHEN document_type.code is not null
                            AND document_type.code = '14'
                            AND am.state != 'cancel' THEN TO_CHAR(am.invoice_date_due, 'DD/MM/YYYY')
                            ELSE ''
                        END AS date_due,
                        ...
                        COALESCE(
                            TO_CHAR(am.origin_invoice_date, 'DD/MM/YYYY'),
                            ''
                        ) AS origin_date_invoice,
                        COALESCE(origin_document_type.code, '') AS origin_document_code,
                        split_part(
                            replace(COALESCE(am.origin_number, ''), ' ', ''),
                            '-',
                            1
                        ) AS origin_serie,
                        split_part(
                        ...
                        (
                            (
                                (
                                    ...
                                    (
                                        (
                                            ...
                                            AND (am."ple_date" >= '{date_start}')
                                        )
                                        AND (am."ple_date" <= '{date_end}')
                                    )
                                    AND (
                                        (
                                            am."state" not in ('{state}')
                                        )
                                        OR am."state" IS NULL
                                    )
                                    )
                                    ...
                                    am."journal_id" in (
                                        ...
                                        WHERE
                                            (
                                                "account_journal"."ple_no_include" IS NULL
                                                or "account_journal"."ple_no_include" = false
                                            )
                                            AND (
                                                "account_journal"."company_id" IS NULL
                                                OR (
                                                "account_journal"."company_id" in (
                                                    {company_id}
                                                )
                                                )
                                            )
                                ...
                                AND (
                                    ...
                                    WHERE
                                        (
                                            "account_journal"."type" = '{journal_type}'
                                        )
                                        AND (
                                            "account_journal"."company_id" IS NULL
                                            OR (
                            ...
                            AND (
                                am."ple_its_declared" IS NULL
                                or am."ple_its_declared" = false
                            )
                        )
                        AND (
                            am."company_id" IS NULL
                            OR (
                                am."company_id" in ({company_id})
                            )
                        )                                                
                    -- ORDER DATA
                    ORDER BY
        ...
        try:
            ...
            raise ValidationError(
                f"Error al ejecutar la query, comunicarse con el administrador: \n {error}"
            )
    ...
    def get_excel_data(self, data_lines):
        ...
        for obj_line in data_lines:
            ...
            value = {
                ...
                "customer_document_number": obj_line.get(
                ...
                "amount_total": obj_line.get("amount_total", ""),
                ...
                "inconsistency_type_change": obj_line.get(
        ...
        self.write(
            {
                ...
                "error_dialog": (
                    "No hay contenido para presentar en el registro de ventas electrónicos de este periodo."
