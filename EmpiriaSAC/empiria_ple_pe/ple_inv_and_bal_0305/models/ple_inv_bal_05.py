...
from ..reports.report_inv_bal_05 import ReportInvBalFourExcel, ReportInvBalFourTxt
import base64


class PleInvBal105(models.Model):
    _name = 'ple.report.inv.bal.05'
    _description = 'Cuentas por Cobrar Diversas de Terceros y Relacionadas'
    _inherit ='ple.report.base'

    line_ids = fields.One2many(
        comodel_name='ple.report.inv.bal.line.05',
        inverse_name='ple_report_inv_val_05_id',
        string='Líneas'
    )
    ...
    txt_binary = fields.Binary(string='Reporte .TXT 3.5')

    pdf_filename = fields.Char(string='Filaname .txt')
    pdf_binary = fields.Binary(string='Reporte .TXT 3.5')
    ...
    def action_generate_report(self):
        ...
        account_ids = self.env['account.account'].search([('group_id.code_prefix_start', 'in', ('16', '17'))])
                ...
                for x in data[k]:
                    ...
                    if len(lines_data) == 0:
                        values = {
                            ...
                            'mont': x['balance'],
                            'status': '1',
                            'ple_report_inv_val_05_id': self.id,
                            'valor': 1,
                        }
                        ...
                        if 'l10n_latam_identification_type_id' in x.keys():
                            ...
                            values['doc_type'] = doc_type.l10n_pe_vat_code
                        if x['vat_ple'] == '0':
                            values['doc_type'] = 0
                        if values['mont'] == 0:
                            values['mont'] = 0.00
                        total_mount_05 = total_mount_05 + x['balance']
        ...
        self.env['ple.report.inv.bal.line.05'].create(lines_data_no)
        self.total_mount = total_mount_05
        self.env['ple.report.inv.bal.line.05'].create(lines_data)
    ...
    def action_generate_excel(self):
        ...
        for rec in self:
            report_name = "ple_inv_and_bal_0305.action_print_status_finance"
            pdf = self.env.ref(report_name)._render_qweb_pdf('ple_inv_and_bal_0305.print_status_finance', self.id)[0]#corregido
            rec.pdf_binary = base64.encodebytes(pdf)
            year, month, day = self.date_end.strftime('%Y/%m/%d').split('/')
            rec.pdf_filename = f'Libro_Cuentas  clientes diversas_{year}{month}.pdf'
...
class PleInvBalLines(models.Model):
    _name = 'ple.report.inv.bal.line.05'
    ...
    ple_report_inv_val_05_id = fields.Many2one(
        comodel_name='ple.report.inv.bal.05',
