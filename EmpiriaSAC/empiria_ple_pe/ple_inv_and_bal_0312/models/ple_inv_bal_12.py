...
from ..reports.report_inv_bal_12 import ReportInvBal12Excel, ReportInvBal12Txt
import re


class PleInvBal12(models.Model):
    _inherit = 'ple.report.inv.bal.one'

    xls_filename_312 = fields.Char(string='Filaname_12 Excel')
    xls_binary_312 = fields.Binary(string='Reporte Excel')
    txt_filename_312 = fields.Char(string='Filaname_12 .txt')
    txt_binary_312 = fields.Binary(string='Reporte .TXT 3.12')
    pdf_filename_312 = fields.Char(string='Filaname_12 .pdf')
    pdf_binary_312 = fields.Binary(string='Reporte .PDF 3.12')

    line_ids_312 = fields.One2many(comodel_name='ple.report.inv.bal.line.12', inverse_name='report_id')

    def action_generate_excel(self):
        super().action_generate_excel()
        data = self.generate_data_report_312()

        report_xls = ReportInvBal12Excel(self, data)
        values_content_xls = report_xls.get_content()
        self.xls_binary_312 = base64.b64encode(values_content_xls)
        self.xls_filename_312 = report_xls.get_filename()

        report_txt = ReportInvBal12Txt(self, data)
        values_content_txt = report_txt.get_content()
        self.txt_binary_312 = base64.b64encode(values_content_txt.encode() or '\n'.encode())
        self.txt_filename_312 = report_txt.get_filename()

        report_name = "ple_inv_and_bal_0312.action_print_status_finance"
        pdf = self.env.ref(report_name)._render_qweb_pdf('ple_inv_and_bal_0312.print_status_finance', self.id)[0]
        self.pdf_binary_312 = base64.encodebytes(pdf)
        self.pdf_filename_312 = f"Libro_Cuentas_por_pagar_{self.date_end.strftime('%Y%m')}.pdf"

        self.line_ids_312.unlink()
    ...
    def generate_data_report_312(self):
        data_account = {}
        objs = self.env['account.account'].search([('group_id.code_prefix_start', 'in', ('42', '43'))])
