...
from ..reports.report_inv_bal_03 import ReportInvBalThreeExcel, ReportInvBalThreeTxt
import base64


class PleInvBal03(models.Model):
    _name = 'ple.report.inv.bal.03'
    _description = 'Cuentas por cobrar'
    _inherit = 'ple.report.base'

    line_ids = fields.One2many(
        comodel_name='ple.report.inv.bal.line.03',
        inverse_name='ple_report_inv_val_03_id',
        string='Líneas'
    )
    ...
    txt_binary = fields.Binary(string='Reporte .TXT 3.3')

    pdf_filename = fields.Char(string='Filaname .pdf')
    pdf_binary = fields.Binary(string='Reporte .PDF 3.3')
    ...
    def action_generate_report(self):
        ...
        account_ids = self.env['account.account'].search([('group_id.code_prefix_start', 'in', ['12', '13'])])
            ...
            if data[k]:
                for x in data[k]:
                    ...
                    if len(lines_data) == 0:
                        ...
                        if values['mont'] == 0:
                            values['mont'] = 0.00
                        total_mount_03 = total_mount_03 + x['balance']
        ...
        self.env['ple.report.inv.bal.line.03'].create(lines_data_no)
        self.total_mount = total_mount_03
        self.env['ple.report.inv.bal.line.03'].create(lines_data)
    ...
    def action_generate_excel(self):
        ...
        for obj_line in self.line_ids:
            ...
            if obj_line.valor == 2:
                ...
                list_data_no.append(values_no)

        report_excel = ReportInvBalThreeExcel(self, list_data_no)
        report_txt = ReportInvBalThreeTxt(self, list_data)
        ...
        for rec in self:
            report_name = "ple_inv_and_bal_0303.action_print_status_finance"           
            pdf = self.env.ref(report_name)._render_qweb_pdf('ple_inv_and_bal_0303.print_status_finance', self.id)[0]
            rec.pdf_binary = base64.encodebytes(pdf)
            year, month, day = self.date_end.strftime('%Y/%m/%d').split('/')
            rec.pdf_filename = f'Libro_Cuentas por Cobrar Clientes_{year}{month}.pdf'
...
class PleInvBalLines(models.Model):
    _name = 'ple.report.inv.bal.line.03'
    ...
    ple_report_inv_val_03_id = fields.Many2one(
        comodel_name='ple.report.inv.bal.03',
